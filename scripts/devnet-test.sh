...
log "✅ 3-node devnet mesh established"
...
log "✅ RSA-4096 key generation successful (all nodes)"
...
log "✅ RSA keys persisted (N1=$COUNT1, N2=$COUNT2)"
...
if python3 -c "import sys; sys.exit(0 if float('$BAL1') >= 10.0 else 1)" 2>/dev/null; then
    ...
    if echo "$ALIAS_TX" | grep -q "error"; then
        ...
        log "⚠️  registerAlias error: $ERROR"
    elif [ "$ALIAS_TX" = "FAIL" ]; then
        log "⚠️  registerAlias failed (may need more confirmations or balance)"
    else
        log "✅ registerAlias successful: $ALIAS_TX"
    fi
    ...
    if echo "$MSG_TX" | grep -q "error"; then
        ...
        log "⚠️  sendPlainMessage error: $ERROR"
    elif [ "$MSG_TX" = "FAIL" ]; then
        log "⚠️  sendPlainMessage failed"
    else
        log "✅ sendPlainMessage successful"
    fi
else
    log "⚠️  SKIP on-chain tests (balance: $BAL1 IOC, need 10+)"
...
RESULTS
-------
✅ 3-node mesh network established
✅ RSA-4096 key generation (all nodes)
✅ RSA key persistence
✅ DIONS RPC commands present
✅ Restart persistence
